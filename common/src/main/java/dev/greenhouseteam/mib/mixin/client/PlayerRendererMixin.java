...
public class PlayerRendererMixin {
    ...
    private boolean mib$modifyToTwoHanded(boolean original, AbstractClientPlayer player) {
        if (player.getItemInHand(player.getUsedItemHand()).has(MibDataComponents.INSTRUMENT)) {
            ItemInstrument component = player.getItemInHand(player.getUsedItemHand()).get(MibDataComponents.INSTRUMENT);
