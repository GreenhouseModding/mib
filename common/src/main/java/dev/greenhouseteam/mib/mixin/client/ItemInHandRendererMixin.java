...
public class ItemInHandRendererMixin {
    ...
    private static ItemInHandRenderer.HandRenderSelection mib$renderInstrumentHandInMainhand(ItemInHandRenderer.HandRenderSelection original, LocalPlayer player) {
        if (player.isUsingItem() && player.getUseItem().getItem() instanceof MibInstrumentItem && player.getUseItem().has(MibDataComponents.INSTRUMENT)) {
            ItemStack stack = player.getUseItem();
            if (stack.get(MibDataComponents.INSTRUMENT).animation().isPresent() && stack.get(MibDataComponents.INSTRUMENT).animation().get().isTwoHanded())
